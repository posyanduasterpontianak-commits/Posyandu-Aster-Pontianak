...
export default async function LoginPage() {
  let totalVisitors = 0;
  try {
    totalVisitors = await prisma.visitor.count({ where: { isActive: true } });
  } catch (err) {
    console.error("[LoginPage] DB fetch error:", err);
  }
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-6 py-8">
        <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="hidden lg:block">
            ...
            {/* Stats bar — Real DB data */}
            <div className="mt-8 flex items-center gap-6 text-center">
              <div>
                <p className="text-2xl font-bold text-slate-800">{totalVisitors}</p>
                <p className="text-xs text-slate-500 mt-0.5">Warga Terdaftar</p>
              </div>
              <div className="w-px h-8 bg-slate-200" />
              <div>
                <p className="text-2xl font-bold text-slate-800">6 SPM</p>
                <p className="text-xs text-slate-500 mt-0.5">Bidang Layanan</p>
              </div>
              <div className="w-px h-8 bg-slate-200" />
              <div>
